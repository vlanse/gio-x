...
import (
	...
	"github.com/vlanse/gio-x/richtext"
)
...
// CommitCurrent copies the state of the Current field and appends it to
...
func (g *gioNodeRenderer) EnsureSeparationFromPrevious() {
	...
	// TODO vlanse: ugly spaces between \n may be removed later, this is possibly GIO bug that consequent \n are not rendered
...
func (g *gioNodeRenderer) renderImage(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if img, ok := node.(*ast.Image); ok {
		// TODO vsemenov download images elsewhere
		...
		if err == nil {
			...
			if err == nil {
				...
				if strings.HasSuffix(url, ".svg") {
					...
					icon.SetTarget(0, 0, 800, 600)
					rgba := image.NewRGBA(image.Rect(0, 0, 800, 600))
					icon.Draw(rasterx.NewDasher(800, 600, rasterx.NewScannerGV(800, 600, rgba, rgba.Bounds())), 1)
					decodedImg = rgba
				} else {
					decodedImg, _, _ = image.Decode(response.Body)
				}

				g.Current.Img = decodedImg
				g.Current.Content = ""
				g.CommitCurrent()
			}
		}
